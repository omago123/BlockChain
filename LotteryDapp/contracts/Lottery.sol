...
contract Lottery {
    ...
    function getPot() public view returns (uint256 pot) {
     ...
     /**
     * @dev 베팅과 정답 체크를 한다. 유저는 0.005 ETH를 보내야 하고, 배팅을 1 byte 글자를 보낸다.
    ...
    function transferAfterPayingFee(address payable addr, uint256 amount) internal returns (uint256)
    {
        ...
        // call, send, transfer
        // if specific reason doesn`t exist when you transfer eth use transfer() for security reason.
    ...
    function distribute() public {
        ...
        for (cur = _head; cur < _tail; cur++) {
            ...
            if (currentBlockStatus == BlockStatus.Checkable) {
                ...
                // Actual Block Hash value is random. so It get trouble of testing so use fixed value to test.
